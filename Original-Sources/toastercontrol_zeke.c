...
#pragma DATA _EEPROM, 	0x00,0x0f,0x00,0x28,0x00,0x5A,0x00,0x96,0x00,0x5A,0x00,0xB4,0x00,0x1E,0x00,0xD7,0x00,0x3C,0x00,0x96,
						...
						//and temp = 40 bytes per program x 2 = 80 bytes (locations 0 - 79). //They are initialized as all 000. 
...
//				a_temp3 = a_temp2;				
		...
		//---------------Main Menu-----------------------
			case 0:
				...
				{
					instruction(CLR_DISP);	//Clear screen and put cursor to 1st position on top line
					printf ("Mode:");		//Display top LCD line of display
					instruction(SET_CURSOR + 64);			//setup lower line of display
					printf ("MM  x  x  Nx/Sel");	//button functions (x means no function)
					prev_state = 0;
				...
				if (button_push == TRUE && choice == 's')	
				{
							...
							else 	//else, must be a short push, just go to the next menu selection possibility
							{
								++mode;
								prev_mode = mode;
								instruction(SET_CURSOR + 7);		//save variable space by sending only the unique parts of the string to the LCD
								if (mode == 6)
		...
		//------------------Manual Mode---------------------------------
			case 1:		//Manual Mode - initially in timer-stop/relay-off condition
				...
				{
					instruction(SET_CURSOR);
					printf ("    0s      000");
					send_tunits();
					instruction(SET_CURSOR + 64);
					printf ("MA R/R R-on  /Ex");
					prev_state = 1;
				...
				if (button_push == TRUE)  
				{
					switch (choice)
					{
						case 'u':		//if Up button short or long push								
							...
							else 							//Up button, short push - start (or restart) or stop timer (toggle each time)
							{
								if (timer_on == FALSE) 
								{
									...
									instruction (SET_CURSOR + 67);			//change run to stop option
									printf ("S");
									timer_on = TRUE;
								}
								else						//timer_on must = TRUE to get here
								{
									...
									instruction (SET_CURSOR + 67);			//change stop to run option
									printf ("R");
									timer_on = FALSE;
								};
							};
						...
						case 'd':		//if Down button short or long push - toggle relay 						
							...
							{
								relay_on = TRUE;
								instruction (SET_CURSOR + 73);		
								printf ("off");		//set 'd' button option to turn relay OFF if pushed
								instruction (SET_CURSOR + 7);	//send warning to screen that relay is on
								printf ("RELAY");
							}
							else					//relay must have been ON to get to here
							{
								...
								instruction (SET_CURSOR + 73);		
								printf ("on ");
								instruction (SET_CURSOR + 7);	//turn off relay warning note
								printf ("     ");
							};	
							break;
...
//		        actual_temp = get_temp();
		...
		//--------------Programmed Mode------------------
			...
			case 3: case 4:		//Program 1 or 2 - stop condition
				...
				{
					instruction(SET_CURSOR);
					printf ("0 000s 000");		//step --- elapsed time --- desired temp@end of this step --- actual temp
					send_tunits();
					...
					instruction(SET_CURSOR + 64);
					printf ("P1 R/R Jmp Ed/Ex");		//run/stop/reset  --- Jump to step --- Edit mode/Exit back to Main Menu
					if (state == 3)
					{
						printf ("P1 R/R Jmp Ed/Ex");		//run/stop/reset  --- Jump to step --- Edit mode/Exit back to Main Menu
						prev_state = 3;
					...
					if (state == 4)
					{
						instruction(SET_CURSOR + 65);
						printf ("2");		//identify this as running Program2
						prev_state = 4;
						eeaddoffset = 40;
					};
...
//				//initialize time and temp values for calculation of temp ramp setting below.
				 	...
				 	if (timer_on == TRUE) 
					{
						...
						{				//load next step time and temp from memory
							...
							instruction (SET_CURSOR);		//update step number on display
							print (bin2hex (step_number));
				...
				{				
					switch (choice)
					{
						...
						case 'd':		// Down button, short push - jump to next step						
							...
							instruction (SET_CURSOR);			//update display for step, time and temp changes
							print (bin2hex(step_number));
...
//always get here from Program mode with display so don't need to update the top line here again
					instruction(SET_CURSOR + 64);
					printf ("E1 U/U D/D Nx/Ex");
					if (state == 13)
					{
//						printf ("E1 U/U D/D Nx/Ex");		//increase value --- decrease value --- Next cursor position/Exit back to Program Mode
						prev_state = 13;
					...
					{	
						instruction(SET_CURSOR + 65);
						printf ("2 ");		//identify this as program 2
						prev_state = 14;
					...
					step_time = get_edata (eeaddress);  //get first step_time from memory
				    step_temp = get_edata (eeaddress+2);  //first step_temp data is 2 bytes past time data
					instruction(SET_CURSOR);	//reposition cursor to beginning of top line to print step number
					print(bin2hex(step_number));
					...
					instruction (SET_CURSOR);	//start with cursor at the "step" position = 0 of 16 total positions in the top LCD row.
					instruction (CUR_BLNK_ON);		//turn on cursor and blinking
					cur_pos = 0;
				};
				...
				{        
					...
					
					{
						case 0:		//cursor is at 0 = "step number" position
							...
							update_display (step_temp, 7);
							instruction(SET_CURSOR);		//put cursor back to the step_number position (0)		
							print(bin2hex(step_number));		//note:cursor automatically shifts right one character after writing!!
							instruction (SET_CURSOR);		//shift it back!
							instruction (CUR_BLNK_ON);		//turn on cursor and blinking						
							break;	//end Case 0
					...
					if (choice == 's')		//if Select button has been pushed.  **don't forget to turn off cursor/blinking upon exit
					{
						...
						{					
							...
							instruction (DISP_ON);		//turns off cursor blinking		
						}	
						else		//Select button, short push - mover cursor to time or temp position
						{
							...
							instruction (SET_CURSOR + cur_pos);	//tell LCD where cursor is
							instruction (CUR_BLNK_ON);		//turn on cursor and blinking
						};
					};
		...
		//--------------Setup Mode--------------------
			...
			case 5:		//Setup Mode - for temp_units (C/F), time_units (SSSs/M:SS), relay_LED (O/O), data logging (O/O), start temp (0,30C)
				...
				if (prev_state != 5)		//first time through, set up display
				{
					...
					instruction(SET_CURSOR + 64);	//write lower line of lcd					
					printf ("SU Up  Dn  Nx/Ex");
					prev_state = 5;
					init_state (FALSE, FALSE);	//initialize common code
					set_mode = 1;
				...
				if (button_push == TRUE)  
				{
					...
					instruction(SET_CURSOR);	//go to beginning of top line of display	
					switch (set_mode)	//update printout for the correct setup variable
					{
						case 1:		//tc = time constant (warm up delay lag)
							...
							printf ("Time Const =   s"); //use string constants - not enough memory for variable storage
							update_display (value, 12);
							break;
						...
						case 2:
							...
							printf ("Temp Units  =  ");
							instruction (SET_CURSOR + 15);
							send_tunits();
							break;
						case 3:
							relay_LED = value;
							printf ("Relay LED  =");
							instruction (SET_CURSOR + 12);
							if (value == TRUE)
								printf (" ON ");
							else printf (" OFF");
							break;
						case 4:
							...
							printf ("Tm/tmp Incr =   ");
							instruction (SET_CURSOR + 14);
							if (value == 9)		
							{
								print ('1');
								print ('0');   //display a hex 10 value since we're only using 1 space
							}							
							else 
								print (bin2hex (value+1)); //t_inc range from 1 to 10
							break;
						case 5:
							clock_cal = value;
							printf ("Clock cal  =    ");
...
void update_display (unsigned int value, unsigned char location)
{
    ...
    instruction(SET_CURSOR + location);		//note: cursor is automatically shifted to the right after each char write
	if (digit_100 == 0) print (' ');  //suppress leading zeros
	else print (bin2hex(digit_100));
	if (digit_100 == 0 && digit_10 == 0) print (' ');
	else print (bin2hex(digit_10));
	print (bin2hex(digit_1));
}	
...
void boot_up(void)
{
	...
	instruction(SET_CURSOR);	//set to beginning of 1st line on LCD	
    printf("SparkFun & Ryan ");
    
    instruction(SET_CURSOR + 64);	//64 = 40H is the beginning of the 2nd line on LCD
    printf(" OvenFlow v1.0  ");
